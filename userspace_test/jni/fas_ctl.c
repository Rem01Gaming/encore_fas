...
/**
 * @brief Converts a jank event type to a text name.
 *
 * @param type The event type from struct fas_jank_event.
...
/**
 * @brief Prints the version of the kernel module.
 *
...
/**
 * @brief Sets the libgui path and the queueBuffer offset in the module.
 *
 * @param fd The file descriptor of the FAS device.
...
static int cmd_set_offset(int fd, int argc, char **argv) {
    ...
    } else if (fas_elf_resolve_offset(path, kQueueBufferSymbols, sizeof(kQueueBufferSymbols) / sizeof(kQueueBufferSymbols[0]), &offset) != 0) {
        fprintf(stderr,
...
/**
 * @brief Prints the libgui path and the queueBuffer offset from the module.
 *
 * @param fd The file descriptor of the FAS device.
 * @return 0 on success. Otherwise 1.
 */
...
/**
 * @brief Attaches a listener to a process and prints the context ID.
 *
 * @param fd The file descriptor of the FAS device.
 * @param argc The number of arguments after the command name.
 * @param argv The arguments after the command name.
...
/**
 * @brief Sets the target frame rate list of a listener. An empty list clears the targets.
 *
 * @param fd The file descriptor of the FAS device.
 * @param argc The number of arguments after the command name.
 * @param argv The arguments after the command name.
 * @return 0 on success. Otherwise 1.
 */
static int cmd_targets(int fd, int argc, char **argv) {
    uint32_t fps[FAS_MAX_TARGETS];

    if (argc < 1) {
        fprintf(stderr, "targets requires a ctx_id\n");
        return 1;
    }

    if (argc - 1 > FAS_MAX_TARGETS) {
        fprintf(stderr, "targets accepts at most %d fps values\n", FAS_MAX_TARGETS);
        return 1;
    }
    ...
    for (uint32_t i = 0; i < count; i++)
        fps[i] = (uint32_t)atoi(argv[i + 1]);

    if (fas_client_set_target_fps_list(fd, atoi(argv[0]), fps, count) != 0) {
        perror("targets ioctl");
        return 1;
    }

    if (count == 0)
        printf("target fps list cleared\n");
    else
        printf("target fps list set: %u entries\n", count);

    return 0;
}

/**
 * @brief Prints jank events until the read fails.
 *
 * @param fd The file descriptor of the FAS device.
 * @return 1 after the read fails.
 */
static int cmd_listen(int fd) {
    struct fas_jank_event ev;

    while (fas_client_read_event(fd, &ev) == 0) {
        printf("ctx=%d type=%-11s ts=%" PRIu64 "ms frametime=%" PRIu64 "ms\n", ev.ctx_id, event_type_name(ev.type), (uint64_t)ev.timestamp_ms, (uint64_t)ev.frametime_ms);
    }
...
int main(int argc, char **argv) {
    ...
    if (strcmp(cmd, "version") == 0)
        ret = cmd_version(fd);
    else if (strcmp(cmd, "set-offset") == 0)
        ...
        ret = cmd_targets(fd, argc - 2, argv + 2);
